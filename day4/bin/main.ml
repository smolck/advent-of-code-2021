...
let make_board (input : string list) =
    let rows = List.map ~f:(String.split ~on:' ') input
               |> List.map ~f:(fun l -> 
...
let read_game file =
    let lines = In_channel.read_all file 
                |> String.split ~on:'\n'
                |> List.filter ~f:(fun x -> not (String.is_empty x)) in (* Filter out empty lines *)
    let boards = List.sub ~pos:1 ~len:(List.length lines - 1) lines
                 |> List.chunks_of ~length:5
                 |> List.map ~f:make_board in
    let draw = List.nth_exn lines 0 
               |> String.split ~on:','
               |> List.map ~f:int_of_string
    in
    { draw; boards }

let max_el_exn l = List.max_elt ~compare:Poly.compare l |> (fun x -> Option.value_exn x)
...
let check_list (draw : int list) (ll : int list list) : int =
    let map_row row =
        let map_num num = 
            let (index, _) = 
                List.findi ~f:(fun _ x -> phys_equal x num) draw
...
let () = 
    ...
    let () = print_endline ("The winning board is " ^ (string_of_int (i + 1)) ^ " in " ^ (string_of_int (num_of_draws + 1)) ^ " draws") in
    let () = print_endline ("Final score of board: " ^ (string_of_int (score game.draw num_of_draws winning_board))) in
    ...
    let () = print_endline ("The last winning board is " ^ (string_of_int (i + 1)) ^ " in " ^ (string_of_int (num_of_draws + 1)) ^ " draws") in
    print_endline ("Final score of board: " ^ (string_of_int (score game.draw num_of_draws winning_board)))
